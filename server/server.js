...
let baseRender = (renderer, pageName, req, res) => {
    ...
    let context = {title: routeConfig.title, url: '/'+url};           //这个context同时会传递给entry-server，server里边可以注入数据，然后再交给模版
    renderer.renderToString(context, (err, html) => {
        if (err) {
            console.log('err ' + err);   //如果entry-server返回的是promise，那么err就是reject的内容
...
for(let rewrite in router) {
  rewrites.push({
    from: `/${rewrite}`,
    to: router[rewrite].url+'#/'
  })
}
